...
const Prejobs = (props) => {
  ...
  const handleAnswerSelection = (category, questionIndex, answer) => {   
  ...
  const questionType = (input, questionIndex, answer)=>{
    if(input === 'Documentazione safety'){
      handleAnswerSelection('safety', questionIndex, answer)
    } else if(input === 'Organizzazione attività'){
      handleAnswerSelection('activities', questionIndex, answer)
    } else if(input === 'Ambiente di lavoro, interferenze, ordine e pulizia'){
      handleAnswerSelection('env', questionIndex, answer)
    } else if(input === 'Metodologie di lavoro'){
      handleAnswerSelection('method', questionIndex, answer)
    } else if(input === 'Verifica DPI/DPC ed attrezzature'){
      handleAnswerSelection('dpiDpcCheck', questionIndex, answer)
    }
  }
            ...
            <Row key={qIndex} className="mt-4 py-4 border-secondary border-bottom">
                ...
                <Col className="ms-3" key={`${qIndex}-b`}>
                    <Row className="w-100 h-100" key={`${qIndex}-c`}>
                        <Col className="d-flex align-items-center justify-content-center me-3 pjAnswer" key={`${qIndex}-d`} onClick={()=>questionType(prejobsQuestions[currentCategoryIndex].description, qIndex, 'Si')} >Si</Col>
                        <Col className="d-flex align-items-center justify-content-center me-3 pjAnswer" key={`${qIndex}-e`} onClick={()=>questionType(prejobsQuestions[currentCategoryIndex].description, qIndex, 'No')}>No</Col>
                        <Col className="d-flex align-items-center justify-content-center pjAnswer" key={`${qIndex}-f`} onClick={()=>questionType(prejobsQuestions[currentCategoryIndex].description, qIndex, 'N.A.')}>N.A.</Col>
                    </Row>
                </Col>
            </Row>
    )
